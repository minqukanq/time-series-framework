...
class SegRNN(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.rnn = nn.GRU(
            input_size=self.d_model, hidden_size=self.d_model, num_layers=1, bias=True, batch_first=True, bidirectional=False
        )
    ...
    def encoder(self, x):
        ...
        pos_emb = (
            torch.cat(
                [self.pos_emb.unsqueeze(0).repeat(self.enc_in, 1, 1), self.channel_emb.unsqueeze(1).repeat(1, self.seg_num_y, 1)],
                dim=-1,
            )
