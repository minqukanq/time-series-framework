...
from layers.decompositions import TrendDecomp, TrendDecompMulti
...
class MIC(nn.Module):
    ...
    def __init__(
        self, feature_size=512, n_heads=8, dropout=0.05, decomp_kernel=[32], conv_kernel=[24], isometric_kernel=[18, 6], device="cuda"
    ):
        ...
        self.decomp = nn.ModuleList([TrendDecomp(k) for k in decomp_kernel])
...
class MICN(nn.Module):
    ...
    def __init__(self, configs, conv_kernel=[12, 16]):
        ...
        self.decomp_multi = TrendDecompMulti(decomp_kernel)
