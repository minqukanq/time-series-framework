...
class ProbAttention(nn.Module):
    ...
    def _update_context(self, context_in, V, scores, index, L_Q, attn_mask):
        ...
        context_in[torch.arange(B)[:, None, None], torch.arange(H)[None, :, None], index, :] = torch.matmul(attn, V).type_as(
            context_in
        )
        if self.output_attention:
...
class TwoStageAttentionLayer(nn.Module):
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None):
        ...
        # Cross Dimension Stage: use a small set of learnable vectors to aggregat, distribute messages to build the D-to-D connection
